...
alias magelog='kontext && kubectl logs -f deployments/magento -c fpm';
alias magedeploylog='kontext && kubectl logs -f --all-containers -l job-name=status-check';
alias magedump='kontext && kubectl exec deployment/magento -c fpm -- magerun2 db:dump --strip="@development akeneo_connector_*" --stdout > ./development.sql && ls -lah development.sql';
alias mageconfig='kontext && kubectl exec deployment/magento -c fpm -- bin/magento app:config:dump && kubectl exec deployment/fpm -- cat app/etc/config.php > ./config.dump.php && ls -lah config.dump.php';
...
alias dmagento='docker exec -w /var/www/magento -it magento';
